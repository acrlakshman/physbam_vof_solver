...
// Class SURFACE_AREA
//#####################################################################
#ifndef __SURFACE_AREA__
#define __SURFACE_AREA__
...
//#####################################################################
    // Compute surface area magnitude at each face
    bool Compute(const GRID<TV>& grid, T_FACE_ARRAYS_SCALAR& surface_area_field);

    // Output area vector at a face
    bool Get_Area_Vector(const GRID<TV>& grid, const T_FACE_ARRAYS_SCALAR& surface_area_field, const T_FACE_LOOKUP& face_lookup, const TV_INT& cell,
                         const int axis, VECTOR<TV,2>& area_vector);
//#####################################################################
};
}
#endif // __SURFACE_AREA__
